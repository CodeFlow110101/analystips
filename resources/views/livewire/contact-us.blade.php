...
<div>
    ...
    <div class="px-4 md:px-12 lg:px-20 xl:px-28 py-20">
        <div class="text-white" style="background-image: url('{{ asset('images/pexels-lusaya-401107.jpg') }}'); background-size: cover; background-position: center; background-repeat: no-repeat;">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 p-8 bg-black/70">
                <div class="flex justify-between items-center gap-4">
                    <div>
                        <svg class="w-14 h-14 text-amber-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
        ...
        <div class="py-20 px-4 md:px-12 lg:px-20 xl:px-28 grid grid-cols-1 gap-2 h-min text-center">
            ...
            <div class="text-2xl sm:text-4xl transition-colors duration-500 font-medium hover:text-amber-500">SEND US A MESSAGE</div>
            <div class="text-sm sm:text-xl font-medium text-black">It would be great to hear from you! If you have any question, please do not hesitate to send us a message. We are looking forward to hearing from you!</div>
        </div>
        <div class="sm:py-20 grid grid-cols-1 gap-4 h-min">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-2 font-medium text-black text-sm">
                <div>
            ...
            <div>
                <textarea wire:model="comment" rows="4" class="block font-medium text-black text-sm bg-gray-100 p-4 w-full outline-none transition-colors transition-shadow duration-500 focus:shadow-lg focus:bg-transparent" placeholder="Comment"></textarea>
                <div class="text-red-500 font-normal text-sm">@error('comment') {{ $message }} @enderror</div>
            </div>
            <div class="flex justify-center">
                <div wire:loading.class="pointer-events-none" wire:target="sendMail" wire:click="sendMail" class="text-center font-medium group text-white uppercase rounded-sm bg-amber-500 whitespace-nowrap py-2 px-6 w-min cursor-pointer transition-colors duration-500 border hover:border-amber-500 hover:bg-white hover:text-amber-500">
                    <div wire:loading.class="hidden" wire:target="sendMail" class="flex justify-between items-center gap-4">
                        <div>Send Message</div>
                        <div>
                            <svg class="w-5 h-5 transition-colors duration-500 text-white group-hover:text-amber-500 rotate-90" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m12 18-7 3 7-18 7 18-7-3Zm0 0v-5" />
                            </svg>
                        </div>
                    </div>
                    <svg wire:loading.class.remove="hidden" wire:target="sendMail" aria-hidden="true" class="w-6 h-6 hidden mx-16 text-amber-500 animate-spin fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
